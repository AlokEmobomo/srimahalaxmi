...
async function getLineItemsByOutSource(OutSourcePK, userPK, tableName) {
  ...
  const params = {
    ...
    ExpressionAttributeValues: {
      ":sk": OutSourcePK, // The value for SK (e.g., projectId)
      ":userpk": userPK, // The value for userPK
      ":pkPrefix": `${OutSourcePK}#L`, // The prefix for PK
    },
  };
...
async function getOutSourceById(OutSourcePK, tableName) {
  ...
  try {
    const data = await docClient.send(new QueryCommand(params));
    return data.Items  ; // Return all items or an empty array
  } catch (err) {
...
module.exports = { getLineItemsByOutSource, getOutSourceById };
