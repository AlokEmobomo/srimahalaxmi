const {
  GetProjectPKByProjectNumber,getLineItemsByProject,
} = require("./GetLineItemsByProjectNumberModel");
...
exports.handler = async (event) => {
  ...
  const { ProjectNumber } = event.queryStringParameters || {};
  console.log("Query Parameters:", event.queryStringParameters);

  console.log("ProjectNumber type:", typeof ProjectNumber);  // Should match the schema type in DynamoDB

  
  try {

    const projectId = await GetProjectPKByProjectNumber(ProjectNumber, userPK, tableName);
    // Fetch line items associated with the project
    const LineItemsResponse = await getLineItemsByProject(projectId.projectPK, userPK, tableName);
