...
async function GetProjectPKByProjectNumber(ProjectNumber, userPK,tableName) {
  
  const indexName = "ProjectNumberIndex";
  const partitionKey = "PONumber";
  ...
  const params = {
    TableName: tableName,
    IndexName: indexName,
    KeyConditionExpression: "PONumber  = :ProjectNumber AND userPK = :userPK",
    ...
    // Check if a project with the specified ProjectNumber was found
    if (result.Items.length === 0) {
     ...
     const projectPK = result.Items[0].PK;

    return { success: true, projectPK };
  } catch (err) {
    console.log("Unable to fetch ProjectPK", err);
    return { success: false, message: err.message };
  }
}

async function getLineItemsByProject(projectId, userPK, tableName) {
  ...
  const params = {
    TableName: tableName,
    IndexName: indexName, // Name of the GSI where SK is the partition key
...
module.exports = { GetProjectPKByProjectNumber, getLineItemsByProject };
