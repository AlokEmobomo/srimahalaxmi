const { getLineItemsByPurchase } = require("./GetLineItemsByPurchaseModel");
...
exports.handler = async (event) => {
  ...
  const { purchaseOrderId } = event.queryStringParameters || {};
  console.log("Query Parameters:", event.queryStringParameters);

  try {
    // Fetch line items associated with the project
    const LineItemsResponse = await getLineItemsByPurchase(
      purchaseOrderId,
      userPK,
      tableName
    );
