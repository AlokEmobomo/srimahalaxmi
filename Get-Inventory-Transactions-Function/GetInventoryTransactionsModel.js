...
async function getAllInventoryTransactions(userPK, tableName, lastEvaluatedKey = null) {
  ...
  try {
    // Ensure GSI exists
    const isGSIActive = await checkAndCreateGSI(tableName, indexName, partitionKey);
  ...
  if (lastEvaluatedKey) {
    ...
    console.error("Unable to get inventory transactions:", err);
    return {
      success: false,
      message: "Error fetching inventory transactions. Please try again later.",
      error: err.message, // Return error details for debugging
    };
  }
}

async function checkAndCreateGSI(tableName, indexName, partitionKey) {
  try {
