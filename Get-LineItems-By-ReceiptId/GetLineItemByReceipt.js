const { getLineItemsByReceipt } = require("./GetLineItemsByReceiptModel");
...
exports.handler = async (event) => {
  ...
  const { receiptId } = event.queryStringParameters || {};
  console.log("Query Parameters:", event.queryStringParameters);

  try {
    // Fetch line items associated with the project
    const LineItemsResponse = await getLineItemsByReceipt(receiptId, userPK, tableName);
